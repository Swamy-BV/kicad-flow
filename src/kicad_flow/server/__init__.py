...
def main() -> None:
    """Run the MCP server or inspect its packaged dependencies."""
    import argparse
    import asyncio
    ...
    ap = argparse.ArgumentParser(
        prog="kicad-flow", description="KiCadFlow MCP server"
    )
    ap.add_argument(
        "command",
        nargs="?",
        ...
        default="serve",
        help="serve MCP (default) or run read-only installation checks",
    )
    ap.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    ap.add_argument(
        "--json", action="store_true", help="emit machine-readable doctor output"
    )
    ap.add_argument(
        "--http",
        action="store_true",
    ...
    if args.command == "doctor":
        from kicad_flow.diagnostics import format_diagnostics, run_diagnostics

        checks = run_diagnostics()
        print(format_diagnostics(checks, as_json=args.json))
    ...
    try:
        ...
        # Ctrl+C is a normal operator shutdown, including in a frozen console.
        return
