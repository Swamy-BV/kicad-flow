...
class NewVia(_StrictModel):
    ...
    net: str = Field(default="", description="Exact net name, or empty for none.")
    diameter: float = Field(default=0.6, gt=0,
...
def move_footprints(path: str,
                    ...
                    dx: float = 0.0, dy: float = 0.0) -> dict[str, Any]:
...
def measure_placement(
    ...
    proposals = tuple(PlacementProposal(
        ref=item.ref, x=item.x, y=item.y, anchor=item.anchor,
        rotation=item.rotation, side=item.side,
...
@mcp.tool(tags=_meta.PCB_INSPECT, annotations=_meta.READ)
def list_board_nets(path: str, net: str = "",
                    include_rules: bool = False) -> dict[str, Any]:
        ...
        if include_rules:
            ...
            policy = {
                ...
                "unassigned_pad_count": sum(
                    not pad.net for footprint in board.footprints()
                    for pad in footprint.pads if pad.kind != "npth"),
                "without_explicit_class": [item.name for item in nets
    ...
    return {"ok": True, "count": len(nets),
            "nets": [n.as_dict() for n in nets], **policy}
...
def render_board_layout(path: str, output_file: str, side: str = "top",
                        dpi: int = 200, copper: bool = True,
                        silkscreen: bool = True,
                        courtyard: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        image = board.render_layout(
            output_file, side=side, dpi=dpi, copper=copper,
...
def query_board_region(path: str, x1: float, y1: float,
    ...
    try:
        ...
        data = _board(path).region(
            x1, y1, x2, y2, layers=tuple(sorted(set(layers or []))),
            include_fills=include_fills,
        )
        scope = json.dumps([_key(path), data["bounds"], data["layers"], include_fills])
        return {"ok": True, **board_history.observe(
            scope, data, since, max_objects, max_bytes)}
    except _ERRORS as exc:
        return _fail(exc)
