...
class Sheet(ABC):
    ...
    def check_layout(self) -> list[LayoutFinding]:
        """Return potential graphical collisions across this sheet hierarchy.

        This is deliberately separate from :meth:`check`: KiCad's ERC proves
        electrical rules, while this method conservatively estimates visible
        field, label and note bounds and intersects them with wires and other
        text. It reports both objects and never decides which one should move.
        """
