...
class FootprintDef:
    ...
    #: Courtyard centre relative to the library footprint origin.
    courtyard_center: Point
    #: Conservative local-space courtyard rectangle, in corner order.
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {"fp_id": self.fp_id, "description": self.description,
                ...
                "courtyard_center": self.courtyard_center.as_dict(),
...
class Footprint:
    ...
    #: Courtyard centre in board coordinates. Prefer this to offset arithmetic.
    courtyard_center: Point
...
class PlacementProposal:
    """One caller-proposed footprint pose for a non-mutating measurement."""
...
@dataclass(frozen=True)
class PlacementOverlap:
    """Two same-side courtyards with a positive intersection area."""
    ...
    def as_dict(self) -> dict[str, str | float]:
        """Return the measured overlap as JSON."""
        return {"first": self.first, "second": self.second,
                "side": self.side, "area": round(self.area, 4)}


@dataclass(frozen=True)
class PlacementEdge:
    """One footprint that is outside or too close to the board edge."""
    ...
    def as_dict(self) -> dict[str, str | float | bool]:
        """Return the edge measurement as JSON."""
        return {"ref": self.ref, "clearance": round(self.clearance, 4),
                "outside": self.outside}


@dataclass(frozen=True)
class PlacementNetLength:
    """A geometric lower bound for connecting all pads of one net."""
    ...
    def as_dict(self) -> dict[str, str | int | float]:
        """Return the net measurement as JSON."""
        return {"net": self.net, "pads": self.pads,
                "length": round(self.length, 3)}


@dataclass(frozen=True)
class PlacementMeasurement:
    """Independent placement measurements, with no weighted quality score."""

    footprint_count: int
    board_area: float
    courtyard_area: float
    courtyard_area_ratio: float
    front_courtyard_area: float
    back_courtyard_area: float
    front_courtyard_area_ratio: float
    back_courtyard_area_ratio: float
    maximum_side_area_ratio: float
    minimum_edge_clearance: float | None
    required_edge_clearance: float
    overlaps: tuple[PlacementOverlap, ...]
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {
            "valid": not self.overlaps and not self.edge_violations,
            "footprint_count": self.footprint_count,
            "board_area": round(self.board_area, 3),
            "courtyard_area": round(self.courtyard_area, 3),
            "courtyard_area_ratio": round(self.courtyard_area_ratio, 4),
            "front_courtyard_area": round(self.front_courtyard_area, 3),
            "back_courtyard_area": round(self.back_courtyard_area, 3),
            "front_courtyard_area_ratio": round(
                self.front_courtyard_area_ratio, 4
            ),
            "back_courtyard_area_ratio": round(
                self.back_courtyard_area_ratio, 4
            ),
            "maximum_side_area_ratio": round(
                self.maximum_side_area_ratio, 4
            ),
            "minimum_edge_clearance": (
                None if self.minimum_edge_clearance is None
                else round(self.minimum_edge_clearance, 4)
            ),
            "required_edge_clearance": self.required_edge_clearance,
