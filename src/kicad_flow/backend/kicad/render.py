...
def render_board_layout(
    ...
    *,
    ...
    """Render a fast, board-cropped orthographic placement view.

    The plotted layers are explicit switches rather than a guessed design
    mode. Fabrication outlines are always present so component bodies remain
    visible; Edge.Cuts is always present so placement can be judged against
    the actual board boundary.
    """
    src = Path(board)
    if not src.is_file():
        raise FileNotFoundError(f"board not found: {src}")
    if side not in ("top", "bottom"):
        raise ValueError("layout side must be 'top' or 'bottom'")
    if dpi < 1:
        raise ValueError("layout dpi must be positive")
    out = Path(output_file)
    if out.suffix.lower() not in (".png", ".svg"):
        raise ValueError("layout output must end in .png or .svg")
    prefix = "F" if side == "top" else "B"
    layers = ["Edge.Cuts", f"{prefix}.Fab"]
    if copper:
        layers.append(f"{prefix}.Cu")
    if silkscreen:
        layers.append(f"{prefix}.Silkscreen")
    if courtyard:
        layers.append(f"{prefix}.Courtyard")
    out.parent.mkdir(parents=True, exist_ok=True)
    if out.suffix.lower() == ".svg":
        return kicad_cli.cli.pcb_svg(
            src, out, layers=tuple(layers), mirror=side == "bottom"
        )
    with tempfile.TemporaryDirectory() as tmp:
        svg = kicad_cli.cli.pcb_svg(
            src, Path(tmp) / f"{src.stem}.svg", layers=tuple(layers),
            mirror=side == "bottom",
        )
        try:
            import pymupdf
        ...
        zoom = dpi / 72.0
        with pymupdf.open(str(svg)) as document:
