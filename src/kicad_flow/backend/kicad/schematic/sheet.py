...
class _Box:
    ...
    def overlaps(self, other: _Box, clearance: float = 0.05) -> bool:
        ...
        return (min(self.right, other.right) - max(self.left, other.left)
                > clearance
                and min(self.bottom, other.bottom) - max(self.top, other.top)
                > clearance)
...
def _text_width(text: str, size: float) -> float:
    ...
    narrow = " .,:;!|'ijlI1()[]{}"
    wide = "MW@%&#QO"
    units = sum(0.34 if c in narrow else 0.9 if c in wide else 0.62
                for c in text)
    return max(size * 0.35, size * units + size * 0.12)


def _text_box(node: Node, content: str | None = None, *,
              rotation_offset: float = 0.0) -> _Box:
    ...
    sides = {_atom(justify, i) for i in range(len(justify.items) - 1)} \
        if justify is not None else set()
    x, y = _f(at, 0), _f(at, 1)
    if "left" in sides:
        x0, x1 = x, x + width
    elif "right" in sides:
    ...
    for px, py in ((x0, y0), (x0, y1), (x1, y0), (x1, y1)):
        dx, dy = px - x, py - y
        corners.append((x + dx * cos + dy * sin,
                        y - dx * sin + dy * cos))
    return _Box(min(p[0] for p in corners), min(p[1] for p in corners),
...
def _segment_crosses_box(a: Point, b: Point, box: _Box) -> bool:
    ...
    low, high = 0.0, 1.0
    for p, q in ((-dx, a.x - left), (dx, right - a.x),
...
def _on_segment(point: Point, a: Point, b: Point, tolerance: float = 0.01) -> bool:
    ...
    if abs(cross) > tolerance:
        return False
    return (min(a.x, b.x) - tolerance <= point.x
            <= max(a.x, b.x) + tolerance
            and min(a.y, b.y) - tolerance <= point.y
...
class KiCadSheet(Sheet):
    ...
    def measure_placement(
        ...
        clone = KiCadSheet(self._path, copy.deepcopy(self._tree), self._paper,
                           self._instance_path)
        clone._defs = dict(self._defs)
        predicted = tuple(
            clone.place(item.lib_id, item.ref, item.at.x, item.at.y,
                        value=item.value, rotation=item.rotation,
                        mirror=item.mirror, unit=item.unit)
            for item in placements
        )
        bounds = tuple(clone._placement_bounds())
        findings = tuple(clone._placement_findings(bounds))
        return PlacementMeasurement(parts=predicted, bounds=bounds,
                                    findings=findings, page_size=clone.size)
    ...
    def _placement_bounds(self) -> list[PlacementBounds]:
        ...
        for node in self._tree.get_all("symbol"):
            ...
            mirror = (_atom(mirror_node, 0)
                      if mirror_node is not None else "") or ""
            left, bottom, right, top = self.symbol(lib_id, unit=unit).bounds
            corners = [_pin_on_sheet(x, y, 0.0, at, rotation, mirror)[0]
                       for x in (left, right) for y in (bottom, top)]
            body = _Box(min(point.x for point in corners),
                        min(point.y for point in corners),
                        max(point.x for point in corners),
                        max(point.y for point in corners))
            found.append(PlacementBounds(
                name=f"{ref}.{unit}", kind="symbol", ref=ref, unit=unit,
                left=body.left, top=body.top, right=body.right,
                bottom=body.bottom))
            for prop in node.get_all("property"):
                ...
                found.append(PlacementBounds(
                    name=f"{ref}.{unit}.{field}", kind="field", ref=ref,
                    unit=unit, left=box.left, top=box.top, right=box.right,
                    bottom=box.bottom))
        return found
    ...
    def _placement_findings(
        ...
        for item in bounds:
            if (item.left >= left_limit and item.top >= top_limit
                    and item.right <= right_limit
                    and item.bottom <= bottom_limit):
                continue
            findings.append(LayoutFinding(
                severity="warning", kind="page_bounds",
                message=f"{item.name} extends outside the drawable page",
                first=item.name, second="page",
                at=Point(min(max((item.left + item.right) / 2, left_limit),
                             right_limit),
                         min(max((item.top + item.bottom) / 2, top_limit),
                             bottom_limit))))
        ...
        for index, first in enumerate(bounds):
            for second in bounds[index + 1:]:
                ...
                findings.append(LayoutFinding(
                    severity="warning", kind=f"{kinds}_overlap",
    ...
    def _wires_between(self, x1: float, y1: float, x2: float,
        ...
        for node in self._tree.get_all("wire"):
            ...
            if len(xy) < 2:
                continue
            ends = ((round(_f(xy[0], 0), 3), round(_f(xy[0], 1), 3)),
    ...
    def check(self) -> list[Finding]:
        ...
        where_by_page: dict[
            str, dict[tuple[float, float], tuple[str, str]]
        ] = {}
        seen: set[Path] = set()
        ...
        def index(sheet: KiCadSheet, page: str) -> None:
            ...
            for part in sheet.parts():
                where[(round(part.at.x, 2), round(part.at.y, 2))] = (
                    part.ref, ""
                )
    ...
    def _visible_text(self) -> list[_VisibleText]:
        ...
        for node in self._tree.get_all("symbol"):
            ...
            for prop in node.get_all("property"):
                ...
                field = _text(prop)
                at_node = prop.get("at")
                out.append(_VisibleText(
                    kind="field",
                    name=f"{ref}.{unit}.{field} {value!r}",
                    box=_text_box(
                        prop, value, rotation_offset=symbol_rotation
                    ),
                    at=Point(_f(at_node, 0), _f(at_node, 1)),
                ))
    ...
    def _check_layout_page(self, page: str) -> list[LayoutFinding]:
        ...
        for item in texts:
            for a, b in wires:
                ...
                findings.append(LayoutFinding(
                    severity="warning", kind=f"{item.kind}_wire_overlap",
                    message=f"{item.name} is crossed by {wire}",
                    first=item.name, second=wire, sheet=page,
                    at=item.box.meeting(_Box(
        ...
        for index, first in enumerate(texts):
            for second in texts[index + 1:]:
                ...
                findings.append(LayoutFinding(
                    severity="warning", kind=f"{kinds}_overlap",
                    message=f"{first.name} overlaps {second.name}",
                    first=first.name, second=second.name, sheet=page,
