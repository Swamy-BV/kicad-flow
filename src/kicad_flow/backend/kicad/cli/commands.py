...
class KiCadCLI:
    ...
    def version(self) -> str:
        """Return the installed KiCad command-line version."""
        proc = self._run(["version"], timeout=15.0)
    ...
    def pcb_svg(self, board: str | Path, output_file: str | Path, *,
                layers: tuple[str, ...], mirror: bool = False) -> Path:
        """Plot selected board layers to one board-cropped SVG."""
        if not layers:
            raise ValueError("SVG export needs at least one board layer")
        args = [
            "pcb", "export", "svg", "--layers", ",".join(layers),
