...
async def build(client: Client) -> int:
    ...
    print(f"ERC on the root: {report.get('errors', '?')} errors, "
          ...
          f"layout {layout.get('errors', '?')}/"
          f"{layout.get('warnings', '?')}; "
    ...
    if grid["spacing_mm"] != 0.1:
        wrong.append("new boards should recommend a 0.1 mm placement grid")
    ...
    await call("set_board_grid", path=board, spacing_mm=0.1)
    ...
    print(
        "placement: "
        f"{placement.get('overlap_count', '?')} courtyard overlap(s), "
        f"{placement.get('edge_violation_count', '?')} edge violation(s), "
        f"{placement.get('courtyard_area_ratio', 0):.1%} courtyard area, "
        f"{placement.get('connection_length', '?')} mm connection estimate"
    )
    ...
    if layout.get("errors") or layout.get("warnings"):
        wrong.append(
            ...
            f"{layout.get('errors', '?')} error(s) and "
            f"{layout.get('warnings', '?')} warning(s)"
        )
    ...
    same("check_sheet clean", report.get("clean"), not report.get("findings"))
    same("check_sheet_layout clean", layout.get("clean"),
         not layout.get("findings"))
    same("placement valid", placement.get("valid"), True)
    same("unrouted complete", left.get("complete"), left.get("count") == 0)
    same("check_board clean", drc.get("clean"), not drc.get("findings"))
    same(
        "placement face areas",
        placement.get("front_courtyard_area", 0)
        + placement.get("back_courtyard_area", 0),
        placement.get("courtyard_area"),
    )
    same(
        "placement maximum face ratio",
        placement.get("maximum_side_area_ratio"),
        max(placement.get("front_courtyard_area_ratio", 0),
            placement.get("back_courtyard_area_ratio", 0)),
    )
    ...
    same("placement courtyard overlaps", placement.get("overlap_count"), 0)
    same("placement edge violations", placement.get("edge_violation_count"), 0)
    same("placement net count", placement.get("net_count"),
         len(nets.get("nets", [])))
    same("placement net reply limit",
         len(placement.get("net_lengths", [])), 5)
    ...
    note_result = await call("add_texts", path=scratch_sch, notes=[{
        "x": 80, "y": 40, "text": "line one\nline two\t(tab)", "size": 1.27}])
    note = note_result.get("notes", [{}])[0]
    crossing = {
        "x1": note.get("x", 80) - 5 * G,
        "y1": note.get("y", 40) - G,
        "x2": note.get("x", 80) + 10 * G,
        "y2": note.get("y", 40) - G,
    }
    made_crossing = await call("add_wires", path=scratch_sch, wires=[crossing])
    made_segment = made_crossing.get("wires", [{}])[0]
    crossing = {
        "x1": made_segment.get("start", {}).get("x"),
        "y1": made_segment.get("start", {}).get("y"),
        "x2": made_segment.get("end", {}).get("x"),
        "y2": made_segment.get("end", {}).get("y"),
    }
    visual = await call("check_sheet_layout", path=scratch_sch)
    ...
    scratch_track_uuid = scratch_track.get("tracks", [{}])[0].get("uuid", "")
    same("add_tracks returns stable uuid", bool(scratch_track_uuid), True)
    listed_track = (await call("list_copper", path=scratch)).get("tracks", [{}])[0]
    same("list_copper keeps track uuid",
         listed_track.get("uuid"), scratch_track_uuid)
    scratch_via = await call("add_vias", path=scratch, vias=[{
        "x": 10, "y": 2, "net": "N1", "diameter": 0.60, "drill": 0.30}])
    scratch_via_uuid = scratch_via.get("vias", [{}])[0].get("uuid", "")
    same("add_vias returns stable uuid", bool(scratch_via_uuid), True)
    ...
    listed_vias = (await call("list_copper", path=scratch)).get("vias", [])
    same("list_copper keeps via uuid",
         scratch_via_uuid in {via.get("uuid") for via in listed_vias}, True)
    ...
    dangling = [f for f in sdrc.get("findings", [])
                if f.get("kind") == "dangling_track_end"]
    same("routing hygiene names dangling track uuid",
         {f.get("uuid") for f in dangling}, {scratch_track_uuid})
    exact = await call("remove_copper", path=scratch, uuid=scratch_track_uuid)
    same("remove_copper deletes exact uuid", exact.get("removed"), 1)
    replacement = await call("add_tracks", path=scratch, tracks=[{
        "x1": 2, "y1": 2, "x2": 18, "y2": 2,
        "layer": "F.Cu", "width": 0.25, "net": "N1"}])
    same("replacement track gets a new uuid",
    ...
    same("centre-anchored placement reports its centre",
         centred.get("footprints", [{}])[0].get("courtyard_center"),
         {"x": 5.0, "y": 15.0})
    same("centre anchor resolves an asymmetric library origin",
         centred.get("footprints", [{}])[0].get("y"), 13.725)
    crowded = await call("measure_placement", path=scratch,
                         edge_clearance=0.2)
    same("placement measurement finds courtyard overlap",
         crowded.get("overlap_count"), 1)
    tentative = await call("measure_placement", path=scratch,
                           edge_clearance=0.2, placements=[{
                               "ref": "DY", "x": 12, "y": 15,
                               "anchor": "courtyard_center"}])
    same("tentative placement is measured without applying it",
         tentative.get("overlap_count"), 0)
    still_there = await call("get_footprint", path=scratch, ref="DY")
    same("tentative placement leaves the board unchanged",
         still_there.get("courtyard_center"), {"x": 5.0, "y": 15.0})
    await call("move_footprints", path=scratch, refs=["DX", "DY"],
               dx=0.5, dy=0.5)
    shifted = await call("get_footprint", path=scratch, ref="DX")
    same("relative cluster move shifts its physical centre",
         shifted.get("courtyard_center"), {"x": 5.5, "y": 15.5})
    layout_preview = OUT / "_scratch-layout.png"
    await call("render_board_layout", path=scratch,
               output_file=str(layout_preview), courtyard=True)
    same("2D placement render exists", layout_preview.is_file(), True)
